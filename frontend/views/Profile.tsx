...
const Profile: React.FC<ProfileProps> = ({ user, onLogout, onUpdate }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      clearSearch();
      // Re-fetch shortlist to update score engagement
      useUniversityStore.getState().fetchShortlist();
      setSaveSuccess(true);
  ...
  const calculateReadiness = () => {
    let score = 0;
    const { shortlist } = useUniversityStore.getState();

    // Mapping formData to match the logic from Dashboard.tsx
    // Core fields (50%)
    if (formData.educationLevel) score += 10;
    if (formData.major) score += 10;
    if (formData.intendedDegree) score += 10;
    if (formData.preferredCountries.length > 0) score += 10;
    if (formData.budgetMax) score += 10;

    // Exams (25%) - Partial credit
    if (formData.ieltsStatus === 'Completed') score += 15;
    else if (formData.ieltsStatus === 'Booked') score += 10;
    else if (formData.ieltsStatus === 'Preparing') score += 5;

    if (formData.greStatus === 'Completed' || formData.greStatus === 'Not Required') score += 10;
    else if (formData.greStatus === 'Booked') score += 7;
    else if (formData.greStatus === 'Preparing') score += 4;

    // SOP (15%) - Partial credit
    if (formData.sopStatus === 'Ready') score += 15;
    else if (formData.sopStatus === 'Draft') score += 8;
  ...
  const isProfileComplete = readinessScore === 100;

  const getProfileStrength = () => {
    const gpa = Number(formData.gpa);
    if (!isNaN(gpa) && gpa > 0) {
      if (gpa >= 3.5) return { label: 'Strong', color: 'text-emerald-500' };
      if (gpa >= 2.5) return { label: 'Average', color: 'text-orange-500' };
        ...
        <div className="space-y-12 selection:bg-orange-500 selection:text-white pb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div className="glass-panel p-10 bg-white/60 backdrop-blur-md border-white rounded-[3.5rem] flex flex-col justify-between shadow-xl shadow-slate-100/50 relative overflow-hidden group">
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="flex flex-col items-end">
                    <span className={`px-5 py-2 ${isProfileComplete ? 'bg-emerald-500' : 'bg-slate-900'} text-white rounded-2xl text-[9px] font-black uppercase tracking-[0.2em] shadow-lg ${isProfileComplete ? 'shadow-emerald-500/10' : 'shadow-slate-900/10'}`}>
                      {isProfileComplete ? 'VERIFIED' : 'SYNCING'}
                    </span>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-[11px] font-black text-slate-800 uppercase tracking-widest">{readinessScore}%</span>
                  </div>
                  <div className="w-full h-3 bg-white/50 border border-white rounded-full overflow-hidden shadow-inner p-0.5">
                    <div
                      className={`h-full ${isProfileComplete ? 'bg-emerald-500' : 'bg-orange-500'} rounded-full transition-all duration-1000 ease-out shadow-sm`}
                      style={{ width: `${readinessScore}%` }}
                    ></div>
                  </div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider text-right flex justify-between">
                    <span>Targeting: {formData.fieldOfStudy || 'Not Set'}</span>
                    <span className={profileStrength.color}>Assessment: {profileStrength.label}</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-3 space-y-8">
              <div className="p-3 bg-white/40 backdrop-blur-xl border border-white/60 rounded-[2.5rem] space-y-2.5 shadow-2xl shadow-slate-200/50 relative overflow-hidden group/sidebar">
                <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-orange-500/5 rounded-full blur-2xl pointer-events-none group-hover/sidebar:bg-orange-500/10 transition-colors"></div>
                {[
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-6 py-5 rounded-[1.8rem] text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500 group/item border ${activeTab === tab.id
                      ? 'bg-slate-900 text-white border-slate-900 shadow-xl translate-x-1.5'
                      : 'text-slate-500 bg-white/30 border-white/40 hover:bg-white hover:text-slate-900 hover:border-orange-200 hover:shadow-lg active:scale-95'
                      }`}
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-9 h-9 rounded-2xl flex items-center justify-center transition-all ${activeTab === tab.id ? 'bg-orange-500 shadow-lg shadow-orange-500/20' : 'bg-white/50 group-hover/item:bg-orange-50'}`}>
                        <span className={`material-symbols-outlined text-[18px] ${activeTab === tab.id ? 'text-white' : 'text-slate-400 group-hover/item:text-orange-500'}`}>
                          {tab.icon}
                        </span>
                      </div>
                      {tab.label}
                    </div>
                    <span className={`material-symbols-outlined text-[16px] transition-all duration-500 ${activeTab === tab.id ? 'opacity-100 translate-x-0' : 'opacity-20 -translate-x-2 group-hover/item:opacity-100 group-hover/item:translate-x-0'}`}>
                      chevron_right
                    </span>
                  </button>
                ))}
                <div className="pt-6 mt-6 border-t border-white/40 px-2 pb-2">
                  <button
                    onClick={onLogout}
                    className="w-full flex items-center gap-4 px-6 py-5 rounded-[1.8rem] text-[10px] font-black text-red-500 bg-red-50/30 border border-red-100/50 hover:bg-red-500 hover:text-white hover:border-red-500 transition-all duration-500 uppercase tracking-[0.2em] shadow-sm hover:shadow-xl hover:shadow-red-500/20 active:scale-95"
                  >
              ...
              <div className="glass-panel p-10 md:p-14 bg-white/60 backdrop-blur-md border-white rounded-[3.5rem] flex flex-col shadow-xl shadow-slate-100/50 relative overflow-hidden h-full">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 mb-16 relative z-10">
                  ...
                  <button
                    onClick={handleSave}
                    disabled={isSaving}
                    className={`px-12 py-5 rounded-[2rem] font-black text-[10px] uppercase tracking-[0.4em] flex items-center justify-center gap-4 transition-all duration-500 group/btn shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] active:scale-95 disabled:opacity-70 disabled:grayscale ${saveSuccess
                      ? 'bg-emerald-500 text-white shadow-emerald-500/20'
                      : 'btn-gradient text-white hover:-translate-y-1.5 hover:shadow-orange-500/30 active:translate-y-0'
                      }`}
                  >
                    <div className="w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center group-hover/btn:scale-110 transition-transform">
                      <span className={`material-symbols-outlined text-[20px] ${isSaving ? 'animate-spin' : ''}`}>
                        {isSaving ? 'refresh' : saveSuccess ? 'check_circle' : 'bolt'}
                      </span>
                    </div>
                    <span>{isSaving ? 'Saving...' : saveSuccess ? 'Saved Securely' : 'Save Changes'}</span>
                  </button>
                </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (val > 4) {
                              setFormData({ ...formData, gpa: '4.0' });
                            } else if (val < 0) {
                              setFormData({ ...formData, gpa: '0.0' });
                            } else {
